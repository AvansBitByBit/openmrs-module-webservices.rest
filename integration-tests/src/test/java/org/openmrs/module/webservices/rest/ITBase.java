...
public abstract class ITBase {
	...
	static {
		String testUrlProperty = System.getProperty("testUrl");
		String testBaseUrlProperty = System.getProperty("testBaseUrl");
		String configuredUrl = testUrlProperty != null ? testUrlProperty : testBaseUrlProperty;
		if (configuredUrl == null) {
			configuredUrl = "http://admin:Admin123@localhost:8080/openmrs";
		}
		try {
			TEST_URL = new URI(configuredUrl);
			...
			String[] userInfo = getCredentials(TEST_URL);
			ADMIN_USERNAME = userInfo[0];
			ADMIN_PASSWORD = userInfo[1];
			RestAssured.authentication = basic(userInfo[0], userInfo[1]);
		}
		catch (URISyntaxException e) {
			throw new RuntimeException("Invalid uri: " + configuredUrl, e);
		}
	}

	private static String[] getCredentials(URI testUrl) {
		String userInfo = testUrl.getUserInfo();
		if (userInfo != null) {
			return userInfo.split(":", 2);
		}

		return new String[] { System.getProperty("testUsername", "admin"), System.getProperty("testPassword", "Admin123") };
	}
		...
		synchronized (serverStartupLock) {
			if (!serverStarted) {
				...
				final String startupUri = TEST_URL.getScheme() + "://" + TEST_URL.getHost()
				        + (TEST_URL.getPort() > 0 ? ":" + TEST_URL.getPort() : "") + TEST_URL.getPath();
