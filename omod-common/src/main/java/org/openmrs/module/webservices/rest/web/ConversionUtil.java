...
public class ConversionUtil {
	...
	public static Object convert(Object object, Type toType) throws ConversionException {
		...
		Class<?> toClass = getRawClass(toType);

		if (Collection.class.isAssignableFrom(toClass) || toClass.isArray()) {
			return convertToCollectionOrArray(object, toType, toClass);
		}
		...
		if (object instanceof String) {
			Object converted = convertFromString((String) object, toClass, toType);
		...
		Object convertedNumber = convertNumber(object, toClass);
		if (convertedNumber != NO_CONVERSION) {
	...
	private static Class<?> getRawClass(Type toType) {
		return toType instanceof Class ? ((Class<?>) toType) : (Class<?>) (((ParameterizedType) toType).getRawType());
	}

	@SuppressWarnings({ "unchecked", "rawtypes" })
	private static Object convertToCollectionOrArray(Object object, Type toType, Class<?> toClass) throws ConversionException {
		if (!(object instanceof Collection))
			throw new ConversionException("Can only convert a Collection to a Collection/Array. Not " + object.getClass()
			        + " to " + toType, null);

		if (toClass.isArray()) {
			return convertToArray((Collection) object, toClass.getComponentType());
		}

		Collection ret = createCollection(toClass);
		if (toType instanceof ParameterizedType) {
			Type targetElementType = ((ParameterizedType) toType).getActualTypeArguments()[0];
	...
	private static Object convertToArray(Collection input, Class<?> targetElementType) throws ConversionException {
	...
	private static Collection createCollection(Class<?> toClass) throws ConversionException {
	...
	@SuppressWarnings("unchecked")
	private static Object convertFromString(String string, Class<?> toClass, Type toType) throws ConversionException {
		...
		if (toClass.isAssignableFrom(Date.class)) {
			...
			return convertStringToClass(string, toType);
		}

		return convertUsingValueOf(string, toClass);
	}

	private static Date convertStringToDate(String string) throws ConversionException {
		...
		throw new ConversionException("Error converting date - correct format (ISO8601 Long): yyyy-MM-dd'T'HH:mm:ss.SSSZ",
		        pex);
	}

	private static Class<?> convertStringToClass(String string, Type toType) throws ConversionException {
		try {
			return Context.loadClass(string);
		}
		catch (ClassNotFoundException e) {
			throw new ConversionException("Could not convert from " + String.class + " to " + toType, e);
		}
	}

	private static Object convertUsingValueOf(String string, Class<?> toClass) {
		try {
	...
	private static Object convertNumber(Object object, Class<?> toClass) {
